<template>
  <div class="customer-page" ref="pageEl" tabindex="-1">

    <!-- ── PAGE HEADER ──────────────────────────────────── -->
    <div class="g-header">
      <div class="g-header-left">
        <h1 class="g-title">Data Pelanggan (Customer)</h1>
    ...
    <div v-if="!hasSearched" class="empty-hint">
      ...
      <p>Ketik nama atau kode customer di atas untuk mencari</p>
    ...
    <template v-else>
      ...
      <div class="result-meta">
        <span class="result-count">
          <b>{{ totalRows }}</b> customer ditemukan
        </span>
          ...
          <tbody v-else-if="pagedRows.length === 0">
            <tr>
              <td colspan="8" class="empty-cell">
                ...
                Tidak ada customer ditemukan untuk "<b>{{ lastSearch }}</b>"
              </td>
            </tr>
          </tbody>
          <tbody v-else>
            ...
            >
              ...
              <td class="col-kredit">
                <span class="harga-val">{{ formatRp(row.limit_kredit) }}</span>
              </td>
              <td class="col-piutang">
                <span class="harga-val">{{ formatRp(row.saldo_piutang) }}</span>
              </td>
              <td class="col-aksi">
                <div class="aksi-wrap">
                  <button
          ...
          <div class="modal-box" role="dialog" :aria-label="modal.title">
            ...
            <form @submit.prevent="submitModal" class="modal-body">
              <div class="mfield-grid">
                <div class="mfield">
                  <label class="mfield-label">Kode Customer <span class="required">*</span></label>
                  <input
                    v-model="form.kode"
                    ref="inputKode"
                    type="text"
        ...
        <div v-if="deleteModal.show" class="modal-overlay" @click.self="deleteModal.show = false">
          <div class="modal-box modal-box--sm" role="alertdialog">
            ...
            <div class="modal-body">
              <p>Yakin ingin menghapus customer <b>{{ deleteModal.row?.nama }}</b>?</p>
...
function onGlobalKey(e) {
  ...
  switch (e.key) {
    ...
    case 'Delete':
      ...
      break
    case 'ArrowUp':
      if (document.activeElement === inputCustomer.value) break
      e.preventDefault()
      moveRow(-1)
      break
    case 'ArrowDown':
...
function selectFromModal(customer) {
  searchModal.show = false
  searchQuery.value = customer.nama
  doSearch(customer.id)
}
...
// SEARCH
// ───────────────────────────────────────────────────────────
async function doSearch(customerId = null) {
  const q = searchQuery.value.trim()
...
async function submitModal() {
  ...
  try {
    const payload = {
      ...
      limit_kredit: Number(form.limit_kredit) || 10000000,
      aktif: true,
    }
    ...
    alert('Gagal menyimpan customer: ' + err.message)
  } finally {
    saving.value = false
  }
}
...
@import '@/assets/Styles/master-data/customer-page.css';
</style>
